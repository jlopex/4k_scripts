...
usage() { echo "Usage: $0 [-i <input_video>] [-o <output_video>] optional {[-l <logo_file>], [-w]}" 1>&2; exit 1; }
...
yuvs_with_logo_and_watermarking() {
    ...
    ffmpeg -i ${INPUT_VIDEO} -loop 1 -i ${LOGO_FILE} -f rawvideo -pix_fmt yuv420p -filter_complex "[0:v] scale=iw*sar:ih , pad=max(iw\,ih*(16/9)):ow/(16/9):(ow-iw)/2:(oh-ih)/2, scale=${OUTPUT_WIDTH}x${OUTPUT_HEIGHT} [scaled]; [1:v] fade=in:${START_LOGO_FRAME}:${FADE_DURATION}:alpha=1, fade=out:${END_LOGO_FRAME}:${FADE_DURATION}:alpha=1 [logo]; [scaled][logo] overlay=W-w-30:H-h-30:shortest=1" -f rawvideo - | NGS_FragmentPreprocessorYUV -w ${OUTPUT_WIDTH} -h ${OUTPUT_HEIGHT} -f ${FRAME_RATE} --A > ${OUTPUT_VIDEO}.yuvA

    ffmpeg -i ${INPUT_VIDEO} -loop 1 -i ${LOGO_FILE} -f rawvideo -pix_fmt yuv420p -filter_complex "[0:v] scale=iw*sar:ih , pad=max(iw\,ih*(16/9)):ow/(16/9):(ow-iw)/2:(oh-ih)/2, scale=${OUTPUT_WIDTH}x${OUTPUT_HEIGHT} [scaled]; [1:v] fade=in:${START_LOGO_FRAME}:${FADE_DURATION}:alpha=1, fade=out:${END_LOGO_FRAME}:${FADE_DURATION}:alpha=1 [logo]; [scaled][logo] overlay=W-w-30:H-h-30:shortest=1" - | NGS_FragmentPreprocessorYUV -w ${OUTPUT_WIDTH} -h ${OUTPUT_HEIGHT} -f ${FRAME_RATE} --APrime > ${OUTPUT_VIDEO}.yuvAP

}

# create a single yuv with logo
yuv_with_logo() {
    ...
    ffmpeg -i ${INPUT_VIDEO} -loop 1 -i ${LOGO_FILE} -f rawvideo -pix_fmt yuv420p -filter_complex "[0:v] scale=iw*sar:ih , pad=max(iw\,ih*(16/9)):ow/(16/9):(ow-iw)/2:(oh-ih)/2, scale=${OUTPUT_WIDTH}x${OUTPUT_HEIGHT} [scaled]; [1:v] fade=in:${START_LOGO_FRAME}:${FADE_DURATION}:alpha=1, fade=out:${END_LOGO_FRAME}:${FADE_DURATION}:alpha=1 [logo]; [scaled][logo] overlay=W-w-30:H-h-30:shortest=1"  ${OUTPUT_VIDEO}.yuv
}

yuvs_with_watermarking() {
    ffmpeg -i ${INPUT_VIDEO} -f rawvideo -pix_fmt yuv420p -vf "scale=iw*sar:ih , pad=max(iw\,ih*(16/9)):ow/(16/9):(ow-iw)/2:(oh-ih)/2, scale=${OUTPUT_WIDTH}x${OUTPUT_HEIGHT}" - | NGS_FragmentPreprocessorYUV -w ${OUTPUT_WIDTH} -h ${OUTPUT_HEIGHT} -f ${FRAME_RATE} --APrime > ${OUTPUT_VIDEO}.yuv
}

# create a single yuv without logo
yuv() {
